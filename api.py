...
def verify_api_key(x_api_key):
    if x_api_key is None:
        raise HTTPException(status_code=401, detail="No API key provided. Add x-api-key to your headers.")
...
def submit_job(request: JobRequest, x_api_key: Optional[str] = Header(None)):
    ...
    if not re.match(r"^[a-zA-Z0-9_-]+$", request.job_id):
        raise HTTPException(status_code=400, detail="Invalid job_id format. Use only alphanumeric characters, dashes, and underscores.")

    if db.job_exists(request.job_id):
        raise HTTPException(status_code=409, detail=f"Job ID '{request.job_id}' already exists.")

    if request.priority not in (1, 2, 3, 4):
        raise HTTPException(status_code=400, detail="Priority must be 1 (Critical), 2 (High), 3 (Normal), or 4 (Low).")
    ...
    return {"status": "queued", "job_id": request.job_id, "priority": request.priority}
...
def get_logs(job_id: str, x_api_key: Optional[str] = Header(None)):
    ...
    return {"job_id": job_id, "status": job["status"], "logs": db.get_log(job_id)}
...
def cancel_job(job_id: str, x_api_key: Optional[str] = Header(None)):
    ...
    if not cancelled:
        raise HTTPException(status_code=400, detail="Job cannot be cancelled. It may already be finished.")
...
def get_my_jobs(x_api_key: Optional[str] = Header(None)):
    ...
    return {"team": team["team_name"], "total_jobs": len(jobs), "jobs": jobs}

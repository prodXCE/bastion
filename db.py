...
def log_event(job_id, level, message):
    ...
    cursor.execute("INSERT INTO events (job_id, level, message, created_at) VALUES (?, ?, ?, ?)", (job_id, level, message, now))
    conn.commit()
    conn.close()
...
def get_events(job_id=None):
    ...
    cursor = conn.cursor()
    if job_id:
        cursor.execute("SELECT * FROM events WHERE job_id = ? ORDER BY created_at ASC", (job_id,))
    else:
...
def create_job(job_id, team_id, repo_url, cmd, priority=3):
    ...
    cursor = conn.cursor()
    now = datetime.utcnow().isoformat()
    cursor.execute("INSERT INTO jobs (job_id, team_id, repo_url, cmd, status, priority, cancelled, created_at) VALUES (?, ?, ?, ?, 'QUEUED', ?, 0, ?)", (job_id, team_id, repo_url, cmd, priority, now))
    conn.commit()
    conn.close()
...
def get_next_queued_job():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM jobs WHERE status = 'QUEUED' AND cancelled = 0 ORDER BY priority ASC, created_at ASC LIMIT 1")
    job = cursor.fetchone()
    if job is None:
        conn.close()
        return None
    cursor.execute("UPDATE jobs SET status = 'RUNNING' WHERE job_id = ?", (job["job_id"],))
    conn.commit()
    conn.close()
    return dict(job)
...
def cancel_job(job_id):
    ...
    if row["status"] == "QUEUED":
        cursor.execute("UPDATE jobs SET cancelled = 1, status = 'CANCELLED' WHERE job_id = ?", (job_id,))
    else:
        cursor.execute("UPDATE jobs SET cancelled = 1 WHERE job_id = ?", (job_id,))
    conn.commit()
    conn.close()
...
def get_jobs_for_team(team_id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM jobs WHERE team_id = ? ORDER BY created_at DESC", (team_id,))
    rows = cursor.fetchall()
    conn.close()
    return [dict(row) for row in rows]
...
def save_log(job_id, output):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT OR REPLACE INTO logs (job_id, output) VALUES (?, ?)", (job_id, output))
    conn.commit()
    conn.close()
...
def save_metrics(job_id, duration_seconds, exit_code, log_size_bytes):
    ...
    cursor = conn.cursor()
    now = datetime.utcnow().isoformat()
    cursor.execute("INSERT OR REPLACE INTO metrics (job_id, duration_seconds, exit_code, log_size_bytes, recorded_at) VALUES (?, ?, ?, ?, ?)", (job_id, duration_seconds, exit_code, log_size_bytes, now))
    conn.commit()
    conn.close()
...
def get_all_metrics():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT m.*, j.status, j.team_id FROM metrics m JOIN jobs j ON m.job_id = j.job_id ORDER BY m.recorded_at DESC")
    rows = cursor.fetchall()
    conn.close()
    return [dict(row) for row in rows]
...
def create_team(team_name):
    ...
    now = datetime.utcnow().isoformat()
    cursor.execute("INSERT INTO teams (team_id, team_name, api_key_hash, created_at) VALUES (?, ?, ?, ?)", (team_id, team_name, hashed_key, now))
    conn.commit()
    conn.close()
...
def get_all_teams():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT team_id, team_name, created_at FROM teams ORDER BY created_at ASC")
    rows = cursor.fetchall()
    conn.close()
    return [dict(row) for row in rows]
...
def setup_host_network():
    ...
    subprocess.run(
        f"ifconfig {BRIDGE_NAME} inet {JAIL_GATEWAY}/24 up",
        shell=True,
        check=True
    )
...
def create_epair(jail_name):
    ...
    subprocess.run(
        f"ifconfig {BRIDGE_NAME} addm {host_side} up",
        shell=True,
        check=True
    )
...
def create(jail_name, root_path):
    ...
    cmd = (
        ...
        f"vnet=new "
        f"vnet.interface={jail_side} "
        f"persist"
    )
    run_command(cmd)

    subprocess.run(
        f"jexec {jail_name} ifconfig {jail_side} name eth0",
        shell=True,
        check=True
    )

    subprocess.run(
        f"jexec {jail_name} ifconfig eth0 inet {jail_ip}/24 up",
        shell=True,
        check=True
    )

    subprocess.run(
        f"jexec {jail_name} route add default {JAIL_GATEWAY}",
        shell=True,
        check=True
    )

...
st.set_page_config(page_title="Bastion CI", page_icon="🏰", layout="wide")
...
def get_job_logs(job_id, api_key):
    try:
        r = requests.get(f"{API_URL}/jobs/{job_id}/logs", headers={"x-api-key": api_key}, timeout=5)
        return r.json()
    except Exception:
...
def submit_job_to_api(job_id, repo_url, cmd, priority, api_key):
    try:
        r = requests.post(f"{API_URL}/jobs", headers={"x-api-key": api_key}, json={"job_id": job_id, "repo_url": repo_url, "cmd": cmd, "priority": priority}, timeout=5)
...
def cancel_job_api(job_id, api_key):
    try:
        r = requests.delete(f"{API_URL}/jobs/{job_id}", headers={"x-api-key": api_key}, timeout=5)
        return r.json(), r.status_code
    except Exception as e:
        return {"detail": str(e)}, 500
...
with st.sidebar:
    ...
    api_key = st.text_input("API Key", type="password", placeholder="bstn_...")
    st.divider()
    st.caption("Bastion CI — Phase 6 Complete")
    auto_refresh = st.checkbox("Auto-refresh every 10 seconds", value=False)

tab1, tab2, tab3, tab4 = st.tabs(["📊 Dashboard", "🚀 Submit Job", "📋 Jobs", "👥 Teams"])
...
with tab2:
    ...
    if not api_key:
        ...
        with st.form("submit_job_form"):
            ...
            priority = st.selectbox("Priority", options=[1, 2, 3, 4], index=2, format_func=lambda x: {1: "1 — Critical", 2: "2 — High", 3: "3 — Normal", 4: "4 — Low"}[x])
            submitted = st.form_submit_button("🚀 Submit Job")
        if submitted:
            if not job_id or not repo_url or not cmd:
                st.error("All fields are required.")
            else:
                result, status_code = submit_job_to_api(job_id, repo_url, cmd, priority, api_key)
                if status_code == 200:
...
with tab3:
    ...
    with col_refresh:
        if st.button("🔄 Refresh"):
            st.rerun()
    with col_filter:
        status_filter = st.selectbox("Filter by status", ["All", "QUEUED", "RUNNING", "SUCCESS", "FAILED", "CANCELLED"])
...
with tab4:
    ...
    if register_btn:
        if not new_team_name:
            ...
            try:
                r = requests.post(f"{API_URL}/teams", json={"team_name": new_team_name}, timeout=5)
                data = r.json()
                if r.status_code == 200:
                    ...
                    st.json({"team_id": data.get("team_id"), "team_name": data.get("team_name")})
                else:
